...
Adapted from the official PFT-SR implementation:
https://github.com/LabShuHangGU/PFT-SR
Pinned source revision: 3f832e6e150ba927a7741d893d3aa4a7114e9b01
...
from neosr.archs.pftsr_smm import (
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, layer_id, window_size, num_heads, num_topk, qkv_bias=True):
        ...
        self.smm_backend: SMMBackend = "torch"
        self.smm_chunk_size = DEFAULT_SMM_CHUNK_SIZE
    ...
    def forward(self, qkvp, pfa_values, pfa_indices, rpi, mask=None, shift=0):
        ...
        # # Sparse Attention Computation using SMM_QmK
            ...
            attn = sparse_qmk(
                q, k, smm_index, self.smm_backend, self.smm_chunk_size
            ).view(b_, self.num_heads, n, topk)
        ...
        if pfa_indices[shift] is None:
            ...
            x = (
                (
                    sparse_amv(
                        attn, v, smm_index, self.smm_backend, self.smm_chunk_size
                    ).view(b_, self.num_heads, n, c // self.num_heads)
                    + v_lepe
                )
                .transpose(1, 2)
...
class PFT(nn.Module):
    ...
    def __init__(
        ...
        upsampler="",
        resi_connection="1conv",
        smm_backend: SMMBackend = "torch",
        smm_chunk_size: int = DEFAULT_SMM_CHUNK_SIZE,
        **kwargs,
    ):
        super().__init__()
        del kwargs
        ensure_smm_backend(smm_backend, smm_chunk_size)
        self.smm_backend = smm_backend
        self.smm_chunk_size = smm_chunk_size
        ...
        for module in self.modules():
            if isinstance(module, WindowAttention):
                module.smm_backend = self.smm_backend
                module.smm_chunk_size = self.smm_chunk_size
...
def _build_pftsr(backend: SMMBackend, lightweight: bool, **kwargs):
    if "smm_backend" in kwargs:
        msg = "Select the PFT-SR backend through its registered architecture name."
        raise TypeError(msg)
    ...
    if lightweight:
        defaults = {
            ...
            "resi_connection": "1conv",
            "use_checkpoint": False,
            "smm_chunk_size": DEFAULT_SMM_CHUNK_SIZE,
        }
    else:
        defaults = {
            "upscale": upscale,
    ...
    defaults["smm_backend"] = backend
    return PFT(**defaults)


@ARCH_REGISTRY.register()
def pftsr(**kwargs):
    """Build the self-contained PFT-SR model with chunked PyTorch SMM."""
    return _build_pftsr("torch", lightweight=False, **kwargs)


@ARCH_REGISTRY.register()
def pftsr_light(**kwargs):
    """Build lightweight PFT-SR with chunked PyTorch SMM."""
    return _build_pftsr("torch", lightweight=True, **kwargs)


@ARCH_REGISTRY.register()
def pftsr_cuda(**kwargs):
    """Build PFT-SR with the optional native CUDA SMM forward backend."""
    return _build_pftsr("cuda", lightweight=False, **kwargs)


@ARCH_REGISTRY.register()
def pftsr_light_cuda(**kwargs):
    """Build lightweight PFT-SR with the optional native CUDA SMM forward backend."""
    return _build_pftsr("cuda", lightweight=True, **kwargs)


__all__ = ["PFT", "pftsr", "pftsr_cuda", "pftsr_light", "pftsr_light_cuda"]
