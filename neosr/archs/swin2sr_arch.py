...
class swin2sr(nn.Module):
    ...
    def __init__(
        ...
        if any(heads <= 0 or embed_dim % heads != 0 for heads in num_heads):
        ...
        if upsampler not in valid_upsamplers:
        ...
        if not isinstance(return_aux, bool):
            raise TypeError("return_aux must be a boolean.")
        num_in_ch = in_chans
        num_out_ch = in_chans
        num_feat = 64
    ...
    def check_image_size(self, x):
        ...
        if mod_pad_h == 0 and mod_pad_w == 0:
            return x
        mode = "reflect" if mod_pad_h < h and mod_pad_w < w else "replicate"
    ...
    def forward(self, x):
        if x.ndim != 4 or x.shape[1] != self.in_chans:
            raise ValueError(
                f"Swin2SR expects BCHW input with {self.in_chans} channels, got {tuple(x.shape)}."
            )
        H, W = x.shape[2:]
        ...
        if self.upsampler == "pixelshuffle_aux":
            main = x[:, :, : H * self.upscale, : W * self.upscale]
            return (main, aux) if self.return_aux else main
            ...
            return outputs if self.return_aux else outputs[1]
